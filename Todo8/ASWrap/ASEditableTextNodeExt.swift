...
extension Reactive where Base: ASEditableTextNode {
    ...
    public func editEvents(_ events: ASEditableTextNode.Event,
                           ...
                           didChangeSelection: ((_ oldRange: NSRange, _ newRange: NSRange, _ dueToEditing: Bool) -> Void)? = nil) -> SignalProducer<ASEditableTextNode.Event, Error> {
    ...
    public var text: SignalProducer<String, Error> {
    ...
    /// Forwards events on main queue.
    /// - Warning: ASEditableTextNode's delegate will be used.
    public var attributedText: SignalProducer<NSAttributedString, Error> {

        return editEvents(.editingChange)
            .filterMap { [weak base] _ in base?.attributedText }
    }
    
    var giaTri: BindingTarget<String> {
        makeBindingTarget { textEdit, giaTri in
            textEdit.attributedText = NSAttributedString(string: giaTri)
        }
    }
}
